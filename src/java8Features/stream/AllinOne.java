...
public class AllinOne {
	public static void main(String[] args) {
		...
		float max=products.stream().map(p1 -> p1.price).max(Float :: compare).get();
		
		Product p = products.stream().max((p1, p2) -> (int) (p1.price - p2.price)).get();
		System.out.println(p);

		System.out.println("******print mini price**********");
		// min() method to get min Product price
		Product productB = products.stream().min((product1, product2) -> product1.price > product2.price ? 1 : -1)
				.get();
